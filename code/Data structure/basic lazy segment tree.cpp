...
public:
    ...
    #define rc (lc | 1)
    struct node {
        ll mn, mx; bool hasLazy;
        node() { mn = mx = 0; hasLazy = false; }
        node(ll _mn, ll _mx, bool _h) : mn(_mn), mx(_mx), hasLazy(_h) {}
    };
    #define MAX_BIT 25
    struct node {
        int bitCount[MAX_BIT];
        node() { memset(bitCount, 0, sizeof(bitCount)); }
    };
    vector<int> a; vector<node> t; int n;
    SegTlazy(int n) {
        this->n = n;
        a.resize(n + 1, 0);
        t.resize(4 * n + 1, node());
    ...
    void merge(node &ND, node L, node R) {
        ...
        while(l < L.vec.size() && r < R.vec.size()) {
            if(L.vec[l] <= R.vec[r]) ND.vec.push_back(L.vec[l++]);
            else ND.vec.push_back(R.vec[r++]);
        }
        while(l < L.vec.size()) ND.vec.push_back(L.vec[l++]);
        while(r < R.vec.size()) ND.vec.push_back(R.vec[r++]);
    }

    // sub array sum
    void merge(node &nd, node l, node r) {
        if(l.tot_sum == inf) { nd = r; return; }
        if(r.tot_sum == inf) { nd = l; return; }
    ...
    void push(int nd, int b, int e) {
        ...
        t[nd] += lazy[nd]; lazy[l] += lazy[nd];
        // multiplication and sum
        t[nd] = (t[nd] * lazy[nd]) % MOD; lazy[l] = (lazy[l] * lazy[nd]) % MOD;
        // addition and sum
        t[nd].val += 1LL * nodes * lazy[nd]; lazy[l] += lazy[nd];
        // assignment and min/max
        t[nd] = lazy[nd]; lazy[l] = lazy[nd];
        // assignment and sum
        t[nd] = 1LL * nodes * lazy[nd]; lazy[l] = lazy[nd];
        // arithmetic progression
        t[nd].mn += lazy[nd]; t[nd].mx += lazy[nd];
        if(b != e) {
            lazy[lc] += lazy[nd]; t[lc].hasLazy = true;
            lazy[rc] += lazy[nd]; t[rc].hasLazy = true;
        }
        lazy[nd] = 0; t[nd].hasLazy = false;
    }
    ...
    int query(int nd, int b, int e, int i, int j, int k) {
        if(j < b || e < i) return 0;
        if(i <= b && e <= j) {
            return t[nd].vec.end() - upper_bound(t[nd].vec.begin(), t[nd].vec.end(), k);
        }
        int mid = (b + e) >> 1;
